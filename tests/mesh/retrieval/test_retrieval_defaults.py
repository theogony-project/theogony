...
def test_the_constants_hold_their_measured_values() -> None:
    ...
    assert DEFAULT_K_SEEDS == 1, (
        "k_seeds is set by a tune/test split on the consolidated founding mesh — k=1 "
        "selected on the tune half in 6 of 6 splits, held-out never worse (PHX-1099); "
        "re-run the protocol before changing it"
    )
