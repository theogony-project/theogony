...
- **Retrieval on that corpus, measured against a gold set rather than by eye.** 32 questions, each carrying the passage that answers it, scored against the live substrate ([`PHX-1067`](phoenix-backlog/PHX-1067.yaml)): every expected entity is present in the mesh (coverage 100%), and recall over them runs **65% at a 30-node constellation and 95% at 200** — the top 4% of the substrate. Honest scope: this measures *retrieval over a corpus the system has read*, not generalisation, and the gold set is 66% genealogical because the corpus is. It exists because three earlier diagnoses drawn from ad-hoc measurement were wrong.
