...
2. Does **Spreading Activation** over a dense vector-graph retrieve better than kNN + heuristic traversal at high edge density? — **measured, and so far not demonstrated**: see [`docs/etappes/qa_benchmark.md`](docs/etappes/qa_benchmark.md) (2Wiki / HotpotQA / PopQA passage recall). Naive SA collapses (confirming the PHX-1042 hub bias); degree-aware SA ties dense kNN without single-hop regression; and **Kadmos-grade LLM edges — 6–9× sparser and typed — do not lift it past parity either**, though they nearly double naive SA. Clean edges and degree normalisation turn out to be *substitutes*. The leading remaining suspect is the **seeding ceiling**: SA is seeded from kNN's own top hits, so it may be structurally unable to beat them.
