...
2. Does **Spreading Activation** over a dense vector-graph retrieve better than kNN + heuristic traversal at high edge density? — *first measurement in [`docs/etappes/qa_benchmark.md`](docs/etappes/qa_benchmark.md) (MuSiQue / 2Wiki / HotpotQA passage recall). First-cut, LLM-free construction: naive SA collapses (confirming the PHX-1042 hub bias), degree-aware SA ties dense kNN without single-hop regression, and no density crossover yet — the predicted lift awaits Kadmos-grade edges.*
