...
<p align="center"><sub><em>Spreading Activation over the <strong>founding mesh</strong>: one query, propagating hop by hop through typed, weighted edges. A real constellation from the Greek-myth corpus that Kadmos read end to end (~5,000 concepts / ~94k edges) — early research. The honest status is at the bottom of this page.</em></sub></p>
...
- **Lifelike dynamics** *(the target — what of it runs today is in the status section below)* — Hebbian strengthening, super-linear decay that spares what fired, bounded saturation, atrophy decoupled from deletion, homeostatic renormalisation.
...
- **What runs today.** An end-to-end path — ingest → embed → sparse-CSR adjacency → Spreading Activation → constellation → a **Cockpit** UI and an **MCP** server. Operator-built Wikidata-seeded subnets up to **100k nodes / ~984k edges**; the activation primitive itself runs **sub-second** at that scale. An Oneiros tick (`theogony mesh tick`) decays edges, merges reinforcement and folds in the activation record; it is invoked by hand, not scheduled. Structured run reports are emitted for every pass.
- **The learning loop is wired, and has beaten once.** A query records which nodes reached its working set (on by default; the evaluation harnesses switch it off) and can credit the edges it traversed (`theogony mesh ask --hebbian`, opt-in); `theogony mesh tick` spares fired edges from decay — the doctrine's own rule, *edges that are not fired weaken* — and folds both records in. Until this was built the substrate could only forget: the strongest reinforcement a single query writes is 17× smaller than one tick of decay, and all 14 ticks the founding mesh had run drained zero reinforcement ([`PHX-1100`](phoenix-backlog/PHX-1100.yaml), [`PHX-1102`](phoenix-backlog/PHX-1102.yaml)). On 2WikiMultihopQA over 50 rounds the substrate now improves on the questions it is used for (+1.3 recall@5) and worsens on the ones it is not (−1.5): it learns from use and displaces what it does not use — the doctrine's homeostatic renormalisation exists for exactly this and is the next organ ([`PHX-1104`](phoenix-backlog/PHX-1104.yaml), [`PHX-1106`](phoenix-backlog/PHX-1106.yaml)). Honest scope: **one-factor Hebbian**, no rater and no consumer-feedback channel, and it cannot create an edge — only strengthen one that ingestion wrote.
- **The founding mesh — read, not seeded.** Beyond the seeded subnets, a dense mesh that **Kadmos read end to end** from the Evelyn-White volume of Hesiod, the Homeric Hymns and Homerica — all 24 works, 1,210 paragraphs, 69,340 words: **~5,000 consolidated nodes / ~94k edges**, read in 1 h 41 min for €0.26. This is the mesh animated at the top of this page and in the [founding demo](demo/founding_demo.md).
- **Retrieval on that corpus, measured against a gold set rather than by eye.** 47 questions covering 92 expected entities (with the spellings each answers to), each question carrying the passage that answers it, scored against the live substrate ([`PHX-1067`](phoenix-backlog/PHX-1067.yaml), widened in [`PHX-1073`](phoenix-backlog/PHX-1073.yaml)): every expected entity is present in the mesh (coverage 98%: two expected answers, `fifty` and `anvil`, are not entities), and recall over them runs **80% at the default** (50-node constellation, one seed), with 34 of 47 questions fully covered — after Oneiros consolidation merged 68 entity candidates into the entities they named ([`PHX-1097`](phoenix-backlog/PHX-1097.yaml)) and the seed count was set on a tune/test split ([`PHX-1099`](phoenix-backlog/PHX-1099.yaml)); it was 74% before either. The budget curve is 64% at 30 and 94% at 200 — the top 4% of the substrate. (All three figures under the gold set repaired in [`PHX-1098`](phoenix-backlog/PHX-1098.yaml); the old gold, with 30 names standing in their own question, read 87%, 39 of 47 and 65/95.) Coverage and recall are reported separately because conflating them produced two of the three wrong diagnoses this instrument exists to prevent. Honest scope: this measures *retrieval over a corpus the system has read*, not generalisation, and the set leans genealogical (55% of questions, 68% of entities) because the corpus does — which is why every result is also reported split by question kind ([`PHX-1080`](phoenix-backlog/PHX-1080.yaml)).
- **What is proven.** The *plumbing* — that the architecture runs end-to-end at ~1M edges.
- **Early signals — internal, single-run, self-measured; suggestive, not conclusive.** On the founding mesh, an in-memory simulation of an Oneiros "dream" pass — the substrate's real tick functions run on a copy, nothing written back — improved held-out link-prediction **MRR by +34.8% (0.44 → 0.60) with no new text**, while a kNN control stayed flat — the substrate's dynamics reorganising *existing* knowledge into better predictions. An LLM judge rated Oneiros-created edges **2.08× more plausible** than degree-matched controls. On a 15k Wikidata subnet, untrained Spreading Activation outranks the untrained kNN and degree baselines on held-out triples (trained KGE models still lead).
...
- **Known limitations, filed openly.** Reading at scale *was* the wall and no longer is: the substrate built no indices at all (PHX-1059), never refreshed the ones it built, accumulated a Lance version pile-up that made every write 30× slower (PHX-1060), and wrote one transaction per audit row (PHX-1061). Per-paragraph resolution went 2.94 s → 0.36 s and the growth curve from quadratic to sublinear, which is what made a full read affordable — and a 6,119-paragraph read then exposed a second quadratic term, one Lance fragment per node until something compacts, so a read now compacts its own workspace every 200 paragraphs and holds 0.80 s per paragraph across the corpus ([`PHX-1110`](phoenix-backlog/PHX-1110.yaml)). Identity was worse than filed: the write path **discarded the entity's name** (PHX-1065), and the mesh held 130 Q-IDs of which 127 were confabulated — Gaia carried the identifier of analytical chemistry (PHX-1063). Both are fixed. What remains open: 93% of edges are asserted without any judgement that a relation exists, under `creation_context` values the doctrine does not list (PHX-1066) — measurement says removing them helps genealogical questions and *hurts* narrative ones, so they stay; the answer turned out to be weighting rather than removing, and ships as a measured, default-off lever ([`PHX-1070`](phoenix-backlog/PHX-1070.yaml)). Consolidation is built and measured (PHX-1097); splits, pathology and therapy remain unbuilt (S5). Of the 119 mechanisms the MESH doctrine specifies, **9 run, 24 in part, 34 are inert, 10 blocked and 42 absent** — the inventory is public ([`docs/etappes/doctrine_inventory.md`](docs/etappes/doctrine_inventory.md), PHX-1100), and what Gen 1 honestly promises after it is written down ([`docs/etappes/what_gen1_promises.md`](docs/etappes/what_gen1_promises.md)). The answer-side instrument on the founding corpus cannot resolve differences under about nine points, because the model has read Hesiod (PHX-1097, PHX-1098). MNLM weight-training is blocked on H100-class compute (PHX-1035).
