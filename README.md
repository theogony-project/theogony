...
- **The learning loop closes.** A query can reinforce the edges it traversed (`theogony mesh ask --hebbian`, off by default so evaluation stays reproducible); the deltas persist and are merged, decayed and saturated by `theogony mesh tick`. Query → reinforcement → tick → denser mesh now runs end to end across processes. Honest scope: this is **one-factor Hebbian** reinforcement, not the doctrine's three-factor rule — there is no rater and no consumer-feedback channel yet, so it strengthens what the substrate already believed.
