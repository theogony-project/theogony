...
Combined reconciliation block for two adjacent merges that share an LLM-default story: PR #30 (provider expansion + first default switch) and the W5 validation PR (live-ingest validation + corrective second default switch, capturing real recording-grade material in [`etappes/demo_log.md`](etappes/demo_log.md)). Plan §5 Week 4 reconciled with the new W5 numbers.

- **PR #30 — provider expansion + first default switch (merged 2026-04-19, recorded retroactively).** Added `OpenAILLMProvider` + `AnthropicLLMProvider` behind the existing `LLMProvider` Protocol; switched the runtime default away from Gemini Flash Lite onto an Anthropic model (initially `claude-3-5-haiku-20241022`, see W5 reconciliation below for why that pick was wrong). Rationale: prepaid Anthropic credits + predictable billing beat Gemini's free-tier 20-RPD daily caps for daily dev work, and the literary-extraction quality differential (German names, transliteration, relation directionality) was Hesiod's PR-30 judgement call. PR #31 (post-merge follow-up bundle) added the per-tier docstring + lint fixes, the Anthropic forced-tool error-path loud-fail, and switched `claude-3-5-haiku-20241022` as the documented default name everywhere (README, `.env.example`, settings, tests). Both merges 100 % CI-green at the time. The Plan §3.3a evaluation table from v3 (Gemini / OpenAI / Claude Haiku 3.5) stays intact as historical evidence of the original three-way decision.

- **W5 finding — `claude-3-5-haiku-20241022` does not exist on the User account.** First W5 smoke-ingest produced 0 edges / €0.00 / 47 audit rows tagged `transport_error:NotFoundError`. Direct probe against the Anthropic API: all `claude-3-5-haiku-*` model IDs return 404; `client.models.list()` shows Opus 4.7 / 4.6 / 4.5, Sonnet 4.6 / 4.5 / 4.x, Haiku 4.5, Haiku 3 — but no Haiku 3.5 anywhere. Anthropic retired the 3.5-Haiku tier between SDK 0.30 (the original PR-30 pin's reference release) and current accounts. PR #30 was never reachable on this User's account; CI didn't catch it because every LLM test was mock-based. **Filed as PHX-0055** ("CI smoke-test against the live default LLM provider") so the next default-swap can't ship silently broken.

- **W5 reconciliation — Hesiod Option A: bump default to `claude-haiku-4-5-20251001`.** Pricing constants in `llm_anthropic.py` updated to the published Haiku 4.5 list (USD/M input 0.80 → 1.00; output 4.00 → 5.00). Forced-tool path validated against both 4.5 and Haiku 3 before commitment — both work cleanly; the PR-30 issue was purely model-availability, not an SDK / schema bug. Doc + test ripple: README quickstart, `.env.example`, `tests/test_settings.py` (default-model assertion + env-override fake), `tests/test_agents_factory.py` (provider construction), `LLMSettings` docstring (added a "PR #30 retired" footnote so future readers see the model-history). Plan §3.3a table left as historical artefact (it captures the decision shape Daedalus walked through, not the specific 2026-04-20 reality).

- **Default raised to Sonnet 4.6 (2026-04-23).** Runtime default Anthropic model is **`claude-sonnet-4-6`** with list pricing USD/M input 3.00 / output 15.00 in `llm_anthropic.py`. Haiku 4.5 remains selectable via `THEOGONY_LLM__MODEL_ID`.

- **W5 finding — full-unbounded ingest of Hedin #43497 is structurally infeasible on Wikidata's free SPARQL endpoint.** Started the W5-brief-spec'd unbounded run (`theogony ingest 43497`); after 45 min the EntityResolver had managed 1 BookContext + 253 Stage-4 LLM calls (€0.51, all 100 % parse-OK) but had not yet exited Stages 1-3 because `query.wikidata.org` was throttling continuously (502 + transport_error retries every few seconds). Linear extrapolation to ~2 200 remaining Stage-4 mentions: ~7.5 hours; way past the W5 brief's 90-min hard-stop ceiling. Anthropic itself was idle most of the run; Wikidata was the bottleneck. Hesiod-decision (relayed via User): kill, document, file backlog evidence, switch to bounded path. **No new PHX needed** — `PHX-0033 — Pre-curated Wikidata subset for travel literature` was already on the Plan §11 list (Daedalus, 2026-04-17); now materialised as a YAML ticket and updated with the measured throttle evidence (pace 5.6 calls / min, projected 7.5 h, 0 nodes-persisted state at kill).

- **W5 bounded ingest — recording-grade material now in place.** `theogony ingest 43497 --sentences 500` (with BookContextExtractor on; the W4 `--no-book-context` was a Gemini-quota hack and is not the production path) completed in **20 min wall-clock for €0.68**, producing **756 nodes / 139 edges / 100 % LLM parse-OK across 371 audit rows**. 4 demo queries (3 substantive + 1 honest-failure) returned **3 good · 1 partial**; the Hover-Lupe walk now shows Sven Hedin (tier-3, `wikidata=Q154759`) ↔ Tibet (tier-4, `wikidata=Q2444884`) with a **12-edge geopolitical neighbourhood** around Tibet (Younghusband / Rawling TRAVELED_TO, John Morley OPPOSED_TO, Imperial Government RULED_BY, Lhasa LOCATED_IN, India NEAR — the Edwardian Tibet expedition map in graph form). 5 OneirosWorker ticks, lifespan shutdown clean. **W5 total spend €1.30**; comfortably under Hesiod's €15 ceiling.

- **Anthropic vs Gemini per-call comparison (from `data/audit.sqlite`).** Per-call: Anthropic Haiku 4.5 is **~21× pricier** (€0.00184 vs €0.0000857), uses **3.4× more input tokens** (1 120 vs 333; mostly forced-tool schema boilerplate plus Anthropic's higher per-token list price), produces **comparable output tokens** (172 vs 148), and is **1.48× slower** (1 829 vs 1 237 ms). Both 100 % parse-OK on this corpus. Quality (entity / relation accuracy) requires PHX-0034's gold-standard benchmark — a one-day dress-rehearsal demo doesn't authoritatively decide which provider is "better". The prepaid-credits-vs-free-tier-cap rationale of PR #30 still holds in absolute-cost terms (€0.68 for the recording corpus), but PHX-0034 evidence may revise the choice in Gen 2.

- **§5 Week 4 — reconciled.** Cardinality success-criterion #3 stays 🟡 with a refined explanation: full-book unbounded is gated by Wikidata throttling (PHX-0033), not by anything in our pipeline; **bounded `--sentences 500` IS the demonstrable cardinality and IS recording-grade** (756 nodes / 139 edges with proper Sven Hedin Wikidata anchor + 12-edge Tibet neighbourhood). Latency criterion #4 stays 🟡 (Mac vs bare-metal-Linux; same explanation as W4). All other ✅ criteria still ✅. The §5 Week-4 deliverables list is otherwise unchanged.

- **PHX tickets in this round.** PHX-0055 NEW (CI smoke-test against live default provider). PHX-0033 materialised from Plan §11 to YAML + updated with W5 throttle evidence (no scope change). No other tickets filed; per the W5 brief's "do not start fixing the Wikidata throttle in this PR" rule.

- **Generation 1's structural surface remains closed.** W5 was a validation + recording-prep stretch, not a structural change. The Plan §1 demonstration moment now has *recording-grade* material; what's left is the User-action 5-min screen recording itself, plus the deferred PHX ladder (PHX-0033 / PHX-0034 / PHX-0035 / PHX-0055).
...
- **§5 E8.5 — SHIPPED.** `src/theogony/memory/oneiros.py` matches the Plan §5 E8.5 `_tick()` pseudo-code line-for-line; `OneirosSettings` lives in `config/settings.py` with the six fields and env-var convention; `KnowledgeStore.count_neighbors_in_layer` and `batch_update_scores` ship on the Protocol with both backends implementing them; `api/app.py:lifespan` populates `app.state.oneiros` + `oneiros_task` with the 5-s graceful-shutdown budget. 28 new tests + 4 extensions, all green on CI's matrix (3.12 + 3.13 + Neo4j-live).
- **PHX-0048 — RESOLVED.** Bulk `batch_update_scores(updates: Sequence[ScoreUpdate])` shipped in the shape Daedalus's E8.5 design picked (vitality precomputed client-side; Cypher only stores the number). `UNWIND $rows AS r MATCH … SET … = COALESCE(...)` Cypher (verbatim from the ticket); InMemory in-process loop. Microbenchmark measured ~6-12× speedup on Mac/testcontainers — same hardware-band-vs-bare-metal-Linux story as PHX-0046; the brief's ≥20× target is bare-metal Linux territory and was lowered to a ≥5× reject floor (Cypher correctness in collapsing round-trips is the contract). PHX-0009 keeps full freedom to evolve the vitality formula because Cypher is formula-agnostic in this shape.
- **PHX-0051 — RESOLVED.** Schema patch (Option A) shipped: `MultiHopBreakdown.nodes_per_hop: list[int] | None` with always-populated `final_node_count: int` companion. `MultiHopRetriever` writes `nodes_per_hop=None` (truthful "store does not expose per-hop visibility") and `final_node_count=len(scored)`. Existing reports on disk parse cleanly (Optional-widening + a regression test pins the legacy parse path).
- **Plan deviation — Q5 convergence test pragmatism.** Plan §5 E8.5's race-condition recipe sketches a `bump` interleaved with the worker mid-tick (snapshot-then-write split). Talos shipped two convergence tests instead of one: (a) `test_bump_during_tick_survives_worker_write` sequences `bump → tick` rather than truly mid-tick (because the in-process `InMemoryKnowledgeStore` does not expose a snapshot-vs-write split point that a unit test could wedge into), and (b) `test_worker_only_writes_connectivity_freshness_vitality` directly inspects the `ScoreUpdate` rows the worker passes to `batch_update_scores` and asserts `confidence is None ∧ relevance is None ∧ connectivity / freshness / vitality non-None`. The second test is the *stronger* contract pin — it verifies the by-construction property that makes the race convergent (the worker never writes relevance) — so the contract is held even though the test sequencing differs from the brief. No PHX ticket; flagged here because it is the only point where the implementation diverges in shape (not in semantics) from the design spec.
- **Plan deviation — `_tick()` `try/finally` for failed-tick reports.** Not in the brief: Talos's `_tick()` wraps the work in `try/finally` so a failed tick still emits one `OneirosTickReport` with `status="failed"` and the partial vitality statistics it gathered before the exception. This matches the Plan §5 E8.5 stance "the lifecycle keeps moving" — operators see failed ticks as report-rows in `data/run_reports/oneiros/`, not just as log lines. Hesiod-judged-acceptable (operator-observability win, no test-locked contract change).
- **Plan deviation — integration-test floor lowered 4 → 3 ticks.** Plan §5 E8.5 success criteria called for "≥4 ticks observed" in 0.5 s. On Mac/testcontainers, each tick body costs ~150–300 ms (multiple Bolt round-trips for `export_layer` + `count_neighbors_in_layer` + `batch_update_scores` + writer.write), so the 0.1-s `tick_interval_s` does not actually pace the loop — the tick body cost does. Talos extended the budget to 1.0 s and lowered the floor to ≥ 3 ticks (the alive-loop signal). CI Linux is faster; brief explicitly allowed this trade-off.
- **PR-bundling pattern continues.** PR #27 = E8.5 + PHX-0048 + PHX-0051 + 9 source files + 8 test files + 3 PHX YAML edits + the etappe brief, in one PR with 1993 additions. Sibling pattern to PR #24 (production-readiness cluster). New workflow ratified by user: Daedalus invoked selectively (used here for the E8.5 design round because Q5 had genuine architectural trade-offs); Talos bundles related work into single larger PRs by default.
- **Zero new PHX tickets** filed in this round. The deviations above are documented in-line; none rise to ticket-worthy follow-up work.
...
- **PHX-0049 — RESOLVED.** `AnswerSynthesizer` system prompt now lives at `src/theogony/retrieval/prompts/answer_synthesizer.md` and loads via `importlib.resources.files()` with explicit `prompt_path` fallback for tests. `pyproject.toml` uses `[tool.hatch.build.targets.wheel.force-include]` to ship the `.md` file in non-editable installs. README quickstart no longer carries the editable-install caveat.
- **PHX-0046 — RESOLVED, with documented threshold deviation.** `KnowledgeStore.batch_upsert_nodes` and `batch_upsert_edges` shipped as part of the Protocol; `Neo4jKnowledgeStore` overrides with a single `UNWIND $rows AS row MERGE …` Cypher per call. `IngestionPipeline._stage_store` now chunks resolved mentions and edges by `Settings.store.batch_size` (default 200). The brief's ≥30× speedup target was lowered to ≥10× (no embedding) / ≥8× (with embedding) / ≥3× (edges) after Talos's empirical Mac/testcontainers measurements showed a hardware-imposed ceiling — the Cypher correctness in collapsing round-trips was confirmed; the original 30× ceiling is bare-metal-Linux-Neo4j territory and is filed as **PHX-0054** for re-measurement.
- **PHX-0042 — RESOLVED, with two follow-up tickets filed.** Cypher query-plan audit complete; results in `docs/cypher_audit/2026-04-19_post_e9.md` with one PROFILE per hot read path (vector_search, traverse, get_neighborhood, multi_hop_search seed, list_pending_resolution). All five paths land at <100 db hits/record and use the expected indexes. Two Gen-2 / opportunistic follow-ups surfaced and are filed: **PHX-0052** (HNSW + filter pushdown — Neo4j 5.x reports the layer/node_type filters as post-filter rather than index-pushed; investigate whether a composite index would change the plan), and **PHX-0053** (`traverse` path-materialisation strips embeddings; saves ~15 % on the wire when the consumer doesn't need them — only worth it if a consumer asks).
- **PHX-0050 — RESOLVED, with semantic refinement.** `KnowledgeStore.get_edges_among(node_ids, min_weight)` shipped on the Protocol; `Neo4jKnowledgeStore` implements it as a single `WHERE a.id IN $ids AND b.id IN $ids` Cypher leveraging the existing range index on `:KnowledgeNode(id)`. `ConstellationAssembler.assemble` is rewired to one bulk call instead of N depth-1 `get_neighborhood` probes. **Semantic refinement:** the original brief assumed orphan-target gap detection would still function under the new Cypher; Talos correctly identified that `WHERE a.id IN $ids AND b.id IN $ids` filters out cross-set edges by construction — orphan-target detection is structurally unreachable from this code path. The `GAP_ORPHAN_PREFIX` constant is preserved for future use; `tests/test_retrieval_constellation.py::test_orphan_target_gap_unreachable_under_bulk_edges_semantics` pins the new contract.
- **§3.1a Convention Decision A — refined.** The original v5 prohibition ("`Settings.neo4j` stays flat. No `Settings.store.neo4j` wrapper. … `StoreSettings` is anticipation for hypothetical future backends and is filed against PHX-0001") stands for *backend-selection wrappers*. PR #24 introduced `Settings.store: StoreSettings` with one field, `batch_size: int = 200`, for the cross-store batch-upsert chunk size (PHX-0046). This is a Gen-1 cross-store performance knob — both `InMemoryKnowledgeStore` and `Neo4jKnowledgeStore` honour it via the new Protocol methods — not the rejected backend-selection anticipation. Cross-store performance knobs are a legitimate exception to Decision A.
...
Canonical **recording-grade** path after W5 / PR #32: bounded ingest of **Gutenberg #43497** (*Trans-Himalaya, Vol. 1*, Sven Hedin), **`--sentences 500`**, BookContextExtractor **on** (not `--no-book-context`). Default LLM: **`claude-sonnet-4-6`**. Unbounded full-book ingest remains gated by Wikidata free-tier SPARQL throttling (**PHX-0033**); see the reconciliation block at the top of this document.
...
- `Settings.neo4j: Neo4jSettings` stays flat, not `Settings.store.neo4j`. Reason: YAGNI. Gen 1 has two `KnowledgeStore` implementations; only Neo4j needs settings. A `StoreSettings` wrapper for hypothetical future backends is anticipation, deferred to PHX-0001 (custom engine) when it ships. **Refined post-PR-#24:** a thin `Settings.store: StoreSettings` group exists since PR #24 carrying exactly one field — `batch_size: int = 200` — for the cross-store batch-upsert chunk size (PHX-0046). This is a Gen-1 cross-store performance knob (both `InMemoryKnowledgeStore` and `Neo4jKnowledgeStore` honour it), not a backend-selection wrapper. The original prohibition stands for backend-selection fields like `Settings.store.neo4j`; cross-store performance knobs are the documented exception.
