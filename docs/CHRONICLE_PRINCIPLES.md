...
11. **Democratic governance — owned by no one** — The chronicle is a commons. No single company, state, or individual owns it; it is governed in the open and stewarded long-term by a foundation rather than a market, with no central gatekeeper deciding what counts as knowledge. Decentralization, local specialization, and sovereign private sub-meshes (see [`PANTHEON_VISION.md`](PANTHEON_VISION.md) §"The Federated Substrate") are first-class, and contradiction stays legible (principle 4) so no single voice can flatten the rest. This is the political spine of the project: **it must always be democratic.**

12. **Self-improving by design — at maximal scale and efficiency** — The system is built to improve *itself*, widening in scope: first its **knowledge** (consolidation — a pass since 2026-08-31, `scripts/mesh_consolidate.py`, not yet part of the tick; the immune system — not yet built, its precondition is PHX-1107), then its own **architecture and implementation**, and ultimately the **stack it runs on** — energy, data centers, chip architecture and fabrication, and beyond. Dedicated agents earn resources for exactly this work, under the safety conditions in [`SELF_MODIFICATION.md`](SELF_MODIFICATION.md) (hard CI wall, bot-account separation, human-review default, reversibility, audit). The concrete implementation is always a *replaceable proposal* in service of the vision — never the point — and maximal scalability and efficiency are themselves vision-level requirements.
