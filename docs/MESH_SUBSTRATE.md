...
4. **`description` is regeneratable and authoritative for its purposes; `description_vector` is the identity-matching surface.** Tier-1 descriptions exist for several consumers: the human debugger reading the mesh, the LLM that needs a short anchor when the activated subgraph is injected, agents performing repairs or disambiguation, the consolidation logic that needs to compare prospective merges, and — crucially — the eager-linking pass that uses `description_vector` for description-based identity matching (per §"Why two tiers — and how identity actually gets committed", signal 2). Descriptions are regenerated by Oneiros from the strongest currently-resonant chunks — concretely: from the **chunk vectors that live in the substrate** plus, when needed, an **off-substrate look-up of the original source text via `raw_text_ref`** (read off the hot path during a tick, never loaded into the mesh as a stored field). `description_generated_at` and `description_source_chunks` make every regeneration auditable. **Substrate logic and consuming agents may read descriptions as authoritative information**, with the understanding that they are summary projections of the underlying chunk evidence — not the chunks themselves. The regenerated `description` itself stays **bounded — typically ~200–500 chars**, and never a stored paragraph body, an extracted source passage, a Wikipedia article body, or any other multi-line block; the substrate's compression discipline applies to descriptions as much as to every other node field. When the description changes (regeneration), `description_vector` is recomputed from it.
...
*Measured 2026-09-12 (PHX-1106, [`etappes/renormalisation.md`](etappes/renormalisation.md)): as written, the correction preserves every **share** as well, and the retrieval operator reads row-normalised adjacency — so on its own it is invisible to retrieval, and with unbounded weights it amplifies the crowding-out that gated decay produces (held-out recall −3.8 against −2.0 without it over 50 rounds of use). The counterforce the paragraph below promises appears only together with a weight cap (§3, built as `w_max` per edge): the scale lifts the unused edges back, the cap stops the used ones from running away. With that, and a set point at 0.9 of the mass the mesh carried when the loop was switched on, use no longer crowds out the rest (+1.5 / −0.7 / +0.7 held-out against −1.5 / −2.3 / −2.3) at about half a point on the used half. The docstring of `renormalise_edges_inplace` carries the three readings.*
