...
| **W17.5-A** | Demo hardening | Repair the local live-demo path before adding new concepts: surface `ingest_failed` reasons, continue through multiple selected research candidates when one fails, add demo startup/worker scripts, and expose a lightweight Cockpit `/health`. | `docs/etappes/W17_5_demo_hardening_brief.md` |
...
**After W18:** the demo should no longer merely show that workers exist. It should show whether knowledge production is good enough: ingest verdict/reasoning, relation counts, slowest stage, pool growth, and bounded runtime must be visible in the Cockpit.
