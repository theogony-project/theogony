...
4. Run **ten queries** through the CLI. Suggested mix (Talos picks the actual ten — the goal is breadth, not these exact strings):
   - 4 fact-recall queries: "Which ethnic groups does Heinrich Harrer describe in his experiences, and by what routes does he encounter them?", "Who is Peter Aufschnaiter?", "When did Harrer reach Lhasa?", "What relationship does Harrer have with the Dalai Lama?".
   - 3 multi-hop queries: "Which people from Harrer's circle are connected to the Tibetan court?", "How is Harrer's escape connected to the Second World War?", "Which places does Harrer visit on his journey from India to Lhasa?".
   - 2 honest-failure queries: questions the book does *not* answer, e.g. "What became of Harrer after 1959?" (book ends earlier), "What musical preferences did Aufschnaiter have?". The verdict should land on `partial` or `inconclusive` with truthful reasoning — *that* is the win, not a hallucinated answer.
